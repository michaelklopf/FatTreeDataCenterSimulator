...
Define_Module(IPECMP);


void IPECMP::initialize()
{
...
void IPECMP::updateDisplayString()
{
    char buf[80] = "";
...
void IPECMP::endService(cPacket *msg)
{
...
InterfaceEntry *IPECMP::getSourceInterfaceFrom(cPacket *msg)
{
    cGate *g = msg->getArrivalGate();
    return g ? ift->getInterfaceByNetworkLayerGateIndex(g->getIndex()) : NULL;
}

void IPECMP::handlePacketFromNetwork(IPDatagram *datagram)
{
    //
    // "Prerouting"
    //
...
void IPECMP::handleARP(ARPPacket *msg)
{
...
void IPECMP::handleReceivedICMP(ICMPMessage *msg)
{
    switch (msg->getType())
    {
...
void IPECMP::handleMessageFromHL(cPacket *msg)
{
...
void IPECMP::routePacket(IPDatagram *datagram, InterfaceEntry *destIE, bool fromHL)
{
    ...
    else
    {
        ...
        routes = dynamic_cast<ECMPRoutingTable *>(rt)->findBestMatchingRoutes(destAddr);

        const IPRoute *re = routes.at(access % routes.size());
        access++;
...
void IPECMP::routeMulticastPacket(IPDatagram *datagram, InterfaceEntry *destIE, InterfaceEntry *fromIE)
{
    IPAddress destAddr = datagram->getDestAddress();
...
void IPECMP::reassembleAndDeliver(IPDatagram *datagram)
{
...
cPacket *IPECMP::decapsulateIP(IPDatagram *datagram)
{
...
void IPECMP::fragmentAndSend(IPDatagram *datagram, InterfaceEntry *ie, IPAddress nextHopAddr)
{
    int mtu = ie->getMTU();
...
IPDatagram *IPECMP::encapsulate(cPacket *transportPacket, InterfaceEntry *&destIE)
{
...
IPDatagram *IPECMP::encapsulate(cPacket *transportPacket, InterfaceEntry *&destIE, IPControlInfo *controlInfo)
{
...
IPDatagram *IPECMP::createIPDatagram(const char *name)
{
    return new IPDatagram(name);
}

void IPECMP::sendDatagramToOutput(IPDatagram *datagram, InterfaceEntry *ie, IPAddress nextHopAddr)
{
    // hop counter check
