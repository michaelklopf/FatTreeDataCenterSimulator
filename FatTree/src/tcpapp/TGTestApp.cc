...
Define_Module(TGTestApp);

void TGTestApp::computeIAT()
{
...
void TGTestApp::initialize(int stage)
{
    if (stage == 0) {
        //double load[3] = {0.2,0.8,0.2};
        double load[1] = {1};
...
void TGTestApp::handleMessage(cMessage *msg)
{
...
void TGTestApp::emitProcessingTime(simtime_t procTime)
{
    emit(processingTime, procTime.dbl());
}

void TGTestApp::emitIAT(double iatValue)
{
    emit(interArrivalTime, iatValue);
}

void TGTestApp::finish()
{
    delete conSock;
    delete loadLevels;
}
